    ...
    public:
                  ...
                  std::size_t width)
        {
            for (std::size_t i = 0; i < height; ++i)
            {
                ...
                {
                    value[row+i][col+j].push_back(id);
                }
            }
        }
        ...
        void countNonOverlap(std::map<std::size_t, std::size_t>& count_map)
        {
            for (auto& row : value)
            {
                for (auto& col : row)
        ...
        std::vector<std::vector<std::vector<std::size_t>>> value;
    };
...
//    std::ifstream input (argv[1]);
    ...
    std::map<std::size_t, std::size_t> total_area_map, non_overlap_map;
...
//        std::cout << '#' << id << " @ " << row << ',' << col << ": " << height << 'x' << width << std::endl;
        ...
        if (id != 0)
        {
            total_area_map[id] = height * width;
            cloth.add (id, row, col, height, width);
        }
    } while(std::cin.good());

    cloth.countNonOverlap(non_overlap_map);

    auto iter = std::find_first_of(non_overlap_map.begin (), non_overlap_map.end (),
                                   total_area_map.begin (), total_area_map.end ());

    if (iter != non_overlap_map.end ())
    {
        std::cout << "Unique id=" << iter->first << std::endl;
    }
    else
    {
