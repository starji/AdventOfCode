...
namespace Intcode
{
    ...
    struct Context
    {
        Context(Memory _memory, Queue& input, Queue& output)
        ...
        std::int64_t& immediate(const std::int64_t index) { return memory[ip() + index]; }
        std::int64_t& reference(const std::int64_t index) { return memory[immediate(index)]; }
        std::int64_t& relative(const std::int64_t index) { return memory[relativeBase() + immediate(index)]; }
