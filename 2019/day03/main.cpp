...
std::vector<coordinate> follow(coordinate origin, std::string command)
{
  ...
  switch(command[0])
    {
    case 'R':
      generate_n(std::back_inserter(result),
		 value,
		 [origin,v=0] () mutable { return std::make_pair(origin.first + ++v, origin.second); });
      break;
    case 'L':
      generate_n(std::back_inserter(result),
		 value,
    ...
    case 'U':
      generate_n(std::back_inserter(result),
		 value,
		 [origin,v=0] () mutable { return std::make_pair(origin.first, origin.second + ++v); });
      break;
    case 'D':
      generate_n(std::back_inserter(result),
		 value,
...
std::vector<coordinate> traceWire(std::vector<std::string> wire_commands)
{
  std::vector<coordinate> wire;
  coordinate last {0, 0};

  for(auto& c : wire_commands)
    {
      auto coords = follow(last, c);
      std::copy(coords.begin (), coords.end (), std::back_inserter(wire));
      last = coords.back();
    }
